...
enum Type {PAUSE, MUTATION, SKILL, END}
...
var popup_end_game: PackedScene = preload("res://ui/popups/popup_end_game/PopupEndGame.tscn")

var popup_dict = {
	Type.PAUSE: popup_pause_scene,
	Type.MUTATION: popup_mutation_selection,
	Type.SKILL: popup_skill_selection,
	Type.END: popup_end_game
}
...
func _ready():
	...
	GameEvents.connect("show_end_game", self, "queue_popup", [Type.END])
...
func _physics_process(delta):
	if popups.size() > 0:
		if canvas:
			...
			elif popups[0] == Type.END:
				popup.set_popup(get_end_stat_ref.call_func())
